...
node<T>* BST<T> :: search(T key){
    ...
    while (temp != NULL && temp->key != key)
    {
        if (key < temp->key)
        {
            //cout<<"SEARCHNG LEFT SUBTREE"<<endl;
            temp = temp->left;
        }
        else if (key > temp->key)
        {
            //cout<<"SEARCHNG RIGHT SUBTREE"<<endl;
            temp = temp->right;
        }
    }

    //cout<<"I AM IN SEARCH FUNCTION IN BST.CPP FILE"<<endl;
    return temp;
}
...
node<T>*  BST<T>::remove(node<T>* p, T k) // k key deletion from p tree
{
    ...
    else
    {
        ...
        // CASE 2: root with 2 children, then we find the in order successor of the node and the smallest key in that subtree and replace it with the node to be deleted
